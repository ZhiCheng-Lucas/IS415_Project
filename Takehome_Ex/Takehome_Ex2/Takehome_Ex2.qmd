...
The following 3D interactive animated plot was created to address the feedback given, specifically the concern about unclear visualization, especially for casual users. The plot aims to present the space-time cube data in a more accessible and interactive format.

In this visualization:

1.  The x and y axes represent the longitude and latitude of Thai provinces, preserving their geographical relationships.

2.  The z-axis, labeled "Number of cases", represents the total number of drug abuse cases. Higher points on this axis indicate a greater number of cases, providing an immediate visual cue for high-incidence areas.

3.  The color of each point corresponds to the number of cases, as shown in the legend on the right. The scale ranges from purple (fewer cases, around 20k) to yellow (more cases, around 60k).

4.  The size of each point is also proportional to the number of cases, offering an additional visual indicator of case volume. Larger circles represent provinces with more cases.

This 3D animated scatter plot presents the space-time cube data in a more intuitive and visually appealing manner. It makes it easier for both casual and experienced users to grasp the geographical distribution of drug abuse cases and how it changes over time across Thailand, addressing the original feedback about visualization clarity and accessibility.
...
plot_data <- data.frame(
  ...
  province = map_data_all$ADM1_EN,
  year = map_data_all$fiscal_year,
  cases = map_data_all$total_cases
)

# Create the 3D animated plot with smaller circles
p <- plot_ly(plot_data, x = ~longitude, y = ~latitude, z = ~cases, 
             color = ~cases, size = ~cases,
             ...
             marker = list(sizemode = "diameter", 
                           sizeref = 2.0,  # Adjust this value to make circles smaller
                           sizemin = 3),   # Set a minimum size for the smallest points
             frame = ~year) %>%
  layout(scene = list(xaxis = list(title = "Longitude"),
                      yaxis = list(title = "Latitude"),
                      zaxis = list(title = "Number of Cases")),
         title = "Drug Abuse Cases in Thailand (2017-2022)")
...
p <- p %>% animation_opts(
  frame = 1000, transition = 500, easing = "cubic-in-out", redraw = FALSE
)
