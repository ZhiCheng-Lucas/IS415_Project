---
title: "Hands-On Exercise 08 & 9: Geographical Segmentation with Spatially Constrained Clustering Techniques"
...
::: callout-note
This includes Hands On 9 below as well.
:::
...
facet_grid(~ CLUSTER) +
theme(axis.text.x = element_text(angle = 30, hjust = 1, size = 6))  # Reduced text size here
```
...
::: callout-note
We will now plot via the different scaling methods below to see the difference.
:::

```{r}
# Standard scaling
plot_std <- ggparcoord(data = shan_sf_ngeo_cluster,
                       columns = c(17:21),
                       scale = "std",
                       alphaLines = 0.2,
                       boxplot = TRUE,
                       title = "Scaling Method: std") +
  facet_grid(~ CLUSTER) +
  theme(axis.text.x = element_text(angle = 30, hjust = 1, size = 8))

# Robust scaling
plot_robust <- ggparcoord(data = shan_sf_ngeo_cluster,
                          columns = c(17:21),
                          scale = "robust",
                          alphaLines = 0.2,
                          boxplot = TRUE,
                          title = "Scaling Method: robust") +
  facet_grid(~ CLUSTER) +
  theme(axis.text.x = element_text(angle = 30, hjust = 1, size = 8))

# Uniminmax scaling
plot_uniminmax <- ggparcoord(data = shan_sf_ngeo_cluster,
                             columns = c(17:21),
                             scale = "uniminmax",
                             alphaLines = 0.2,
                             boxplot = TRUE,
                             title = "Scaling Method: uniminmax") +
  facet_grid(~ CLUSTER) +
  theme(axis.text.x = element_text(angle = 30, hjust = 1, size = 8))

# Globalminmax scaling
plot_globalminmax <- ggparcoord(data = shan_sf_ngeo_cluster,
                                columns = c(17:21),
                                scale = "globalminmax",
                                alphaLines = 0.2,
                                boxplot = TRUE,
                                title = "Scaling Method: globalminmax") +
  facet_grid(~ CLUSTER) +
  theme(axis.text.x = element_text(angle = 30, hjust = 1, size = 8))

# Center scaling
plot_center <- ggparcoord(data = shan_sf_ngeo_cluster,
                          columns = c(17:21),
                          scale = "center",
                          alphaLines = 0.2,
                          boxplot = TRUE,
                          title = "Scaling Method: center") +
  facet_grid(~ CLUSTER) +
  theme(axis.text.x = element_text(angle = 30, hjust = 1, size = 8))

# CenterObs scaling
plot_centerObs <- ggparcoord(data = shan_sf_ngeo_cluster,
                             columns = c(17:21),
                             scale = "centerObs",
                             alphaLines = 0.2,
                             boxplot = TRUE,
                             title = "Scaling Method: centerObs") +
  facet_grid(~ CLUSTER) +
  theme(axis.text.x = element_text(angle = 30, hjust = 1, size = 8))
